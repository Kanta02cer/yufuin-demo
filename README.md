...
| 施設 | 取得可能なソース（実キーで検証済み） |
|------|----------------------------------------|
| 界 由布院 | じゃらん / SerpAPI（※楽天トラベルには非掲載） |
| 亀の井別荘 | 一休スクレイパ → 手動CSV / SerpAPI（※楽天トラベルには非掲載） |
| ENOWA YUFUIN | **楽天トラベル公式API(hotelNo=187963)** ✅実取得成功 / じゃらん / SerpAPI |

各日付の価格は `SOURCE_PRIORITY`（`serpapi_google > rakuten > jalan > ikyu > manual`）の
順で採用され、レポート上に**採用ソースを明示**します。

> **在庫の実態（実キー検証で判明）**: 界由布院・亀の井別荘は**楽天トラベルに在庫が無い**
> （高級旅館のため楽天非掲載）。楽天APIで取得できるのは ENOWA(187963) のみ。
> 施設ページURLの `HOTEL/NNNNN` 番号はAPIの `hotelNo` とは**別物**である点にも注意。
...
APIキーは**コード・リポジトリに絶対にコミットしません**。読み込みは環境変数経由のみです
（`core/config.py`）。キーが未設定のソースは安全に無効化されます。

| 用途 | 方法 | Gitへの露出 |
|------|------|-------------|
| ローカル実行 | 直下に `.env` を作成（`cp .env.example .env` して値を記入） | ❌ `.gitignore` 済みで非追跡 |
| 本番(GitHub Actions) | リポジトリ **Secrets** に登録 | ❌ 暗号化保存・ログにマスク表示 |

### GitHub Secrets への登録手順（本番）

リポジトリ → **Settings** → **Secrets and variables** → **Actions** → **New repository secret**
で以下を登録します（ワークフローは設定済みで自動的に環境変数として渡ります）:

| Secret 名 | 値 | 対象 |
|-----------|----|----|
| `RAPIDAPI_KEY` | RapidAPI のキー | Booking.com |
| `RAKUTEN_APP_ID` / `RAKUTEN_ACCESS_KEY` | 楽天のキー | 楽天(ENOWA等) |
| `SERPAPI_KEY` | SerpAPI のキー | Google Hotels |
| `SLACK_WEBHOOK_URL` | Webhook URL | 通知 |

Referer/Origin など秘密でない設定は **Variables** タブ（`RAKUTEN_REFERER` 等）に登録できます。

> 🔐 **キーがチャットや画面共有で一度でも露出したら、必ず発行元で Regenerate（再発行）**
> してから登録してください。露出したキーは第三者に悪用され課金される恐れがあります。
...
1. https://webservice.rakuten.co.jp/ でアプリID + アクセスキーを発行
2. Developer Console の **Allowed websites** に呼び出し元ドメインを登録
   （例: `*.github.io`）。**2026年新仕様で必須**
3. Actions Secret / Variable に登録:
   - `RAKUTEN_APP_ID`（必須）
   - `RAKUTEN_ACCESS_KEY`（必須・2026年新仕様）
   - `RAKUTEN_REFERER` / `RAKUTEN_ORIGIN`（登録ドメインと一致させる。既定は
     `https://kanta02cer.github.io/`）

**2026年新仕様の要点（実装済み）**: `accessKey` は**HTTPヘッダ**で送信し、
`Origin`/`Referer` が登録ドメインと一致する必要があります（不一致だと
`403 REQUEST_CONTEXT_BODY_HTTP_REFERRER_MISSING`）。エンドポイントは
`openapi.rakuten.co.jp/engine/api/...`。対象施設番号は `core/config.py` の
`RAKUTEN_HOTEL_NOS`（ENOWA=187963）で管理します。
...
- **亀の井別荘・界由布院の取得**: 両施設とも**楽天トラベルに在庫が無い**ことが実キー検証で判明。
  - 亀の井別荘: 一休(ikyu)スクレイパ＋`data/kamenoi_manual.csv`、または SerpAPI(Google Hotels)。
  - 界由布院: じゃらん、または SerpAPI。
  安定運用には SerpAPI（Booking系集約）の併用が最も確実です。
- **ENOWA YUFUIN**: 楽天トラベルAPI(187963)で**実取得を確認済み**（例 2026-08-01 ¥81,890/2名）。
- **じゃらんの取得安定性**: プランコード/DOMの変更で取得0件になる日があります。
- 取得失敗は障害保護（前回データ保護）と Slack 通知で検知できます。
